...
object WindingInsulationTest : KSPADTest(
    abbr = "ВИУ",
    tag = "VIU",
    name = "Испытание электрической прочности изоляции обмотки относительно корпуса и между обмотками",
    tables(
        table1(
            columns(
    ...
    init {
        define(
            initVariables = {
                voltage.value = TIManager.testItem.hvVoltage
                current.value = TIManager.testItem.hvCurrent
                testTime.value = TIManager.testItem.hvTestTime
            },
            assemblyCircuit = {
                DD2.onHV()
                loadUZ91()
